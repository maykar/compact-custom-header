...
if (obj.root) {
  ...
  obj.edit_mode =
    obj.toolbars.length > 1 && obj.toolbars[1].style.cssText != 'display: none;'
      ? true
      : false
  ...
  if (!obj.config.disable && !obj.raw_config) {
    ...
    element_style(obj.config.menu, menu_btn, true)
    element_style(obj.config.notification, notify_btn, true)
    element_style(obj.config.voice, voice_btn, true)
    element_style(obj.config.options, options_btn, true)
    ...
    if (obj.tabs) {
      ...
      let pad = 0
      pad +=
        obj.config.notification && obj.config.clock != 'notification' ? 45 : 0
      ...
      obj.root.querySelector('app-toolbar').style.cssText = 'margin-top:-64px;'
...
function element_style(config, element, shift) {
  ...
  let options_style =
    element == options_btn ? 'margin-right:-5px; padding:0;' : ''
